...
export default class Linus {
    constructor(gui) {
        ...
        this.selectionHelper = new SelectionHelper(this.fourPointSelection.bind(this))

        // Members related to VR
...
/*
        ...
        for (
            ...
            */
            for (
                ...
                geometry.setAttribute(
                    this.shaders.cleanVarName(attName),
                    new Float32BufferAttribute(values, aDim)
                );
                let id = this.shaders.getDataAttributeIndex(
                    i,
                    this.data.sets[i].states[j].attributes[a].name
                );

                for (let vv = 0; vv < values.length; vv++) {
                    this.shaders.updateDataAttributeMinMax(id, values[vv]);
                }
            }
    ...
    addDataHelperTriangles(i, k, dim, numStates) {
        ...
        for (
            ...
            for (
                ...
                for (let vv = 0; vv < values.length; vv++) {
                    this.shaders.updateDataAttributeMinMax(a, values[vv]);
                }
            }
        }
    ...
    preprocessData(data, i) {
        ...
        this.shaders.addCustomAttribute(
            i,
            "vec3",
            "position",
            "stateInternal",
    ...
    processAttributes(setId) {
        let attributes = this.shaders.getAttributesOfSet(setId);
        for (let a = 0; a < attributes.length; a++) {
            let attribute = attributes[a];
            let aDim = attribute.dim;
            let name = attribute.name;
            let shared = attribute.shared;
            let dataset = attribute.dataset;
            let type = "";
            if (aDim == 1) {
                type = "float";
            }
            ...
            this.uniforms[attribute.dataset][
                this.shaders.cleanVarName(attribute.name + "From")
            ] = new Uniform(attribute.min);
            this.uniforms[attribute.dataset][
                this.shaders.cleanVarName(attribute.name + "To")
            ] = new Uniform(attribute.max);
            this.uniforms[attribute.dataset][
                this.shaders.cleanVarName(attribute.name + "Min")
            ] = new Uniform(attribute.min);
            this.uniforms[attribute.dataset][
                this.shaders.cleanVarName(attribute.name + "Max")
            ] = new Uniform(attribute.max);
        }
    }
    ...
    getLineUniforms() {
        return {
            ...
            colorMode: new Uniform(0),
            opacityMode: new Uniform(0),
    ...
    // Holds default values for the uniforms of a line geometry
    getLineMaterial(setId) {
        let setCount = this.setsAndStates.length;
        return new ShaderMaterial({
            uniforms: this.uniforms[setId],
            transparent: true,
            linewidth: 2,
            depthFunc: AlwaysDepth,
            blending: NormalBlending,
            fragmentShader: this.shaders.createFragmentShader(setId, setCount),
            vertexShader: this.shaders.createVertexShaderLine(setId, setCount)
        });
    }
    // Holds default values for the uniforms of a triangle geometry
    getTriangleMaterial(setId) {
        let setCount = this.setsAndStates.length;
        return new ShaderMaterial({
            uniforms: this.uniforms[setId],
            transparent: true,
            ...
            blending: NormalBlending,
            fragmentShader: this.shaders.createFragmentShader(setId, setCount),
            vertexShader: this.shaders.createVertexShaderTriangle(setId, setCount)
    ...
    initGui() {
        ...
        for (let setId = 0; setId < this.setsAndStates.length; setId++) {
            ...
            for (let i = 0; i < attributes.length; i++) {
                ...
                let name = i + ": " + attribute.name;

                // Parameter object, for this specific attribute
                let p = {};
                p.name = name;
                p.uniformName = this.shaders.cleanVarName(attribute.name);
                p.i = i;
                p.to = parseFloat(attribute.max);
                ...
                this.gui.addFloat(
                    setId + "_" + name + " __Min",
                    parseFloat(attribute.min),
                    parseFloat(attribute.max),
                    parseFloat(attribute.min),
                    function (val) {
                ...
                this.opacityModeOptions.push(name);
            ...
            this.gui.addSelection(
                ...
                0,
                function (val) {
                    this.material.uniforms.colorMode.value = val;
                }.bind(p)
            );

            this.gui.addSelection(
                setId.toString() + "__Opacity mode",
                this.opacityModeOptions,
                0,
                function (val) {
                    this.material.uniforms.opacityMode.value = val;
                }.bind(p)
            );
    ...
    onmousedown(e) {
        if (this.webVr) {
            ...
            console.log("some click", e.target)
            if (this.renderer.domElement !== e.target) {
                // Never notice this click if it wasn't targeted at the canvas
                return;
            }
            document.activeElement.blur();
            console.log("some click on canvas")
            this.selectionHelper.handleMouseDown(e);
        }
    }

    // Handler for mouse motion
    onmousemove(e) {
        if (this.clickingVrOnCanvas) {
            ...
            this.selectionHelper.handleMouseMove(e);
        }
    }
    ...
    onmouseup(e) {
        if (this.clickingVrOnCanvas) {
            ...
            this.selectionHelper.handleMouseUp(e);
        }
    }
    ...
    fourPointSelection(fromX, fromY, toX, toY) {
  ...
  */
    ...
    onDocumentKeyDown(event) {
        ...
        if (keyCode == 83) {
            this.selectionHelper.enableSelectionMode();
            this.controls.enabled = false;
            return false;
        }
        return true;
    ...
    onDocumentKeyUp(event) {
        ...
        if (keyCode == 83) {
            this.selectionHelper.disableSelectionMode();
